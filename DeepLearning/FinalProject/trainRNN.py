...
def getMini_batch(x,y, ref, batch_size):
    ...
    refMini = np.empty([1,1,500,1])
    yMini = np.ndarray([1,img_size])
    ...
    refsub = np.expand_dims(x[indx], axis = 0)
...
#     layer_fc1 = tf.nn.dropout(layer_fc1, keep_prob=0.75)
    ...
    with tf.Session() as sess:
        ...
        # now going to test ability on "DSP" signal
        feed_dict = {x: dsp_x, ref: dsp_ref}
